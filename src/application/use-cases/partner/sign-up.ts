...
export class SignPartnerUpUseCase {
  ...
  async execute(props: SignPartnerUpDto): Promise<Error | SignPartnerUpReturn> {
    let inviteToken: InviteToken | null = null
    let affiliate: Affiliate | null = null

    if (props.invite_token) {
      inviteToken = await this.inviteTokenRepo.findByToken(props.invite_token)
    }

    if (props.referral_code) {
      affiliate = await this.affiliateRepo.findByReferralCode(props.referral_code)
    }

    if (!inviteToken && !affiliate) {
      return new Error("Invite token and affiliate not found")
    }
    ...
    const partner = new Partner({
      ...
      operator_id: inviteToken ? inviteToken?.operator_id : undefined,
      affiliate_id: affiliate ? affiliate.id : undefined
    })
